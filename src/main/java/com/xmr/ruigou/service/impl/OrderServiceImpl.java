...
import com.xmr.ruigou.dao.*;
import com.xmr.ruigou.pojo.*;
import com.xmr.ruigou.service.IOrderService;
import com.xmr.ruigou.util.BigDecimalUtil;
import com.xmr.ruigou.util.DateTimeUtil;
import com.xmr.ruigou.util.FTPUtil;
import com.xmr.ruigou.util.PropertiesUtil;
import com.xmr.ruigou.vo.OrderItemVo;
import com.xmr.ruigou.vo.OrderVo;
import com.xmr.ruigou.vo.ShippingVo;
...
public class OrderServiceImpl implements IOrderService {
    ...
    @Override
    public ServerResponse pay(Long orderNum, Long userId, String path) {
        Map<String, String> resultMap = Maps.newHashMap();
        Order order = orderMapper.selectByUserIdAndOrderNum(userId, orderNum);
        if (order == null) {
            return ServerResponse.createByErrorMessage("用户没有该订单");
        }
        resultMap.put("orderNum", String.valueOf(order.getOrderNum()));

        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        String outTradeNum = order.getOrderNum().toString();


        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店当面付扫码消费”
        String subject = new StringBuilder().append("happymmall扫码支付,订单号:").append(outTradeNum).toString();
        ...
        String body = new StringBuilder().append("订单").append(outTradeNum).append("购买商品共").append(totalAmount).append("元").toString();
        ...
        List<OrderItem> orderItemList = orderItemMapper.getByOrderNumAndUserId(orderNum, userId);
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                .setSubject(subject).setTotalAmount(totalAmount).setOutTradeNo(outTradeNum)
    ...
    @Override
    public ServerResponse<PageInfo> getOrderList(Long userId, int pageNum, int pageSize) {
        return null;
    }
    ...
    private OrderVo assembleOrderVo(Order order, List<OrderItem> orderItemList) {
        ...
        orderVo.setOrderNo(order.getOrderNum());
    ...
    private OrderItemVo assembleOrderItemVo(OrderItem orderItem) {
        ...
        orderItemVo.setOrderNo(orderItem.getOrderNum());
    ...
    private void cleanCart(List<Cart> cartList) {
        for (Cart cart : cartList) {
            cartMapper.deleteByPrimaryKey(cart.getProductId());
        }
    }
